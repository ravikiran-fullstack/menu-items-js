...
function generateCategoryBtns() {
  // const filteredBtns = menu.reduce(
...
breakfast.addEventListener("click", function () {
  const breakfastItems = menu.filter(function (item) {
    return item.category === "breakfast";
  });
  displayMenuItems(breakfastItems);
});

lunch.addEventListener("click", function () {
  const lunchItems = menu.filter(function (item) {
...
shakes.addEventListener("click", function () {
  const shakesItems = menu.filter(function (item) {
    return item.category === "shakes";
  });
  displayMenuItems(shakesItems);
});

dinner.addEventListener("click", function () {
  const dinnerItems = menu.filter(function (item) {
...
function displayMenuItems(menuItems) {
  let displayMenu = menuItems.map(function (item) {
    return `<article class="menu-item">
          <img src="${item.img}" alt="buttermilkPancakes"
            class="photo">
          <div class="item-info">
            <header>
              <h4>${item.title}</h4>
